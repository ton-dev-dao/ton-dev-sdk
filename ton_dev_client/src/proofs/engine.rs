...
use ton_dev_block::{BinTreeType, Block, Deserializable, InRefValue, ShardIdent, ShardStateUnsplit};
...
impl ProofHelperEngineImpl {
    ...
    pub async fn check_shard_block(&self, boc: &[u8]) -> Result<()> {
        let cell = ton_dev_block::boc::read_single_root_boc(&boc)?;
        ...
        loop {
                ...
                ).await?
            {
                ...
                let mc_cell = ton_dev_block::boc::read_single_root_boc(&mc_boc)?;

...
use std::str::FromStr;
use ton_dev_block::{MsgAddrStd, MsgAddressInt, Serializable};
use ton_dev_block::{AccountId, BuilderData, IBitstring};
...
fn parse_pruned_account() {
    ...
    let (account, _) = deserialize_object_from_boc_bin::<ton_dev_block::Account>(include_bytes!("test_data/account.boc")).unwrap();
    ...
    let proof = ton_dev_block::MerkleProof::create(
        &cell,
